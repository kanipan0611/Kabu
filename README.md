...
- `stock_analysis.py` — 株価チャート取得（1日／1週間／1ヶ月～2年）・移動平均線・RSI・配当利回り（自動取得）・PER/PBR/ROE（自動取得）・関連ニュース＆業界情報（自動取得、Claudeによる参考見解）・企業名から銘柄コードを検索・初心者向けヒント・2銘柄比較モード
...
- `secrets_utils.py` — `st.secrets`／環境変数からAPIキーを取得する共通ヘルパー
